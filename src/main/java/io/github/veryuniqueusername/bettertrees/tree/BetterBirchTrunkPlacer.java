...
public class BetterBirchTrunkPlacer extends TrunkPlacer {
    ...
    public static final Codec<BetterBirchTrunkPlacer> CODEC = RecordCodecBuilder.create(instance ->
            fillTrunkPlacerFields(instance).apply(instance, BetterBirchTrunkPlacer::new));

    public BetterBirchTrunkPlacer(int baseHeight, int firstRandomHeight, int secondRandomHeight) {
        this(baseHeight, firstRandomHeight, secondRandomHeight, 0.75D, 2D, 0, 0, 0D, 1D, 0D, 1D);
    }

    public BetterBirchTrunkPlacer(int baseHeight, int firstRandomHeight, int secondRandomHeight, double branchProbabilityModifier, double subBranchProbabilityDivisor, int branchLengthModifier, int initialBranchLengthModifier, double minLeftBias, double maxLeftBias, double minUpBias, double maxUpBias) {
    ...
    protected TrunkPlacerType<?> getType() {
        return BetterTreesConfiguredFeatures.BETTER_BIRCH_TRUNK_PLACER;
    }
    ...
    protected class Branch {
        ...
        public List<FoliagePlacer.TreeNode> generate() {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                // add foliage nodes
                if(nodesAllAlong && (random.nextDouble() < 0.95 || i == length - 1))
                    list.add(new FoliagePlacer.TreeNode(bendPos(startPos, i).up(), 0, false));
                else if(i > (length - 5) && level == 0) // generate more leaves at the top of the trunk
