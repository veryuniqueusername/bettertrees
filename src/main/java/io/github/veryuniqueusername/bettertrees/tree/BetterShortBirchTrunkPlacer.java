...
public class BetterShortBirchTrunkPlacer extends TrunkPlacer {
	...
	protected TrunkPlacerType<?> getType() {
		return BetterTreesConfiguredFeatures.BETTER_SHORT_BIRCH_TRUNK_PLACER;
	}
	...
	protected class Branch {
		...
		public List<FoliagePlacer.TreeNode> generate() {
			...
			for (int i = 0; i < length; ++i) {
				...
				// add foliage nodes
				if ((i > (length - 3) || i > 3) && level == 0) // generate more leaves at the top of the branch
				...
				if ((random.nextDouble() < getBranchProbability(i, length, branchProbabilityModifier, clampBelow)) && (level < maxLevel)) {
					int newLength = random.nextInt(3) + 1;
