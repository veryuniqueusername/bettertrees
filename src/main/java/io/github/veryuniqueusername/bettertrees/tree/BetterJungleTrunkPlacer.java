...
public class BetterJungleTrunkPlacer extends TrunkPlacer {
    ...
    public BetterJungleTrunkPlacer(int baseHeight, int firstRandomHeight, int secondRandomHeight, double branchProbabilityModifier, double subBranchProbabilityDivisor, int branchLengthModifier, int initialBranchLengthModifier, double minLeftBias, double maxLeftBias, double minUpBias, double maxUpBias) {
    ...
    protected TrunkPlacerType<?> getType() {
        return BetterTreesConfiguredFeatures.BETTER_JUNGLE_TRUNK_PLACER;
    }

    @Override
    public List<JungleFoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
    ...
    protected class Branch {
        ...
        public List<JungleFoliagePlacer.TreeNode> generate() {
            List<JungleFoliagePlacer.TreeNode> list = new ArrayList<>();
            for (int i = 0; i < length; ++i) {
                ...
                if(nodesAllAlong && (random.nextDouble() < 0.75 || i == length - 1))
                    list.add(new JungleFoliagePlacer.TreeNode(bendPos(startPos, i).up(), 0, false));
                else if(i == (length - 1) && level == 0) // generate more leaves at the top of the trunk
                    list.add(new JungleFoliagePlacer.TreeNode(bendPos(startPos, i).up(), 2, false));
                updateBend();
