					...
					if (i < 5) {
						for (int j = 2; j < 6; ++j) {
							...
							if (random.nextDouble() < 0.5D) {
								...
								if (world.testBlockState(rootPos.down(), Feature::isSoil) || (world.testBlockState(rootPos.down(), blockState -> blockState.isOf(Blocks.SPRUCE_WOOD)) && !world.testBlockState(rootPos.down(), blockState -> blockState.get(PillarBlock.AXIS) == Direction.Axis.Y))) {
									getAndSetState(world, replacer, random, rootPos, config, blockState -> blockState.with(PillarBlock.AXIS, direction.getAxis()));
								}
							}
							if (random.nextDouble() < 0.5D) {
